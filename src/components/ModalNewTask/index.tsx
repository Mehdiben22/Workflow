...
const ModalNewTask = ({ isOpen, onClose, id }: Props) => {
  ...
  //bringing status enum from the state and having as default status.todo
  const [status, setStatus] = useState<Status>(Status.ToDo);
  //bringing priority enum from the state and having as default Priority.Backlog
    ...
    <Modal isOpen={isOpen} onClose={onClose} name="Create New Task">
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-2">
          ...
          value={status}
          //onchange depend on the enum selection status 
          ...
          className={selectStyles}
          value={priority}
          //onchange depend on the enum selection priority 
        ...
        <button
          ...
          className={`focus:offset-2 mt-4 flex w-full justify-center rounded-md border border-transparent bg-blue-primary px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 ${
            !isFormatValid() || isLoading ? "cursor-not-allowed opacity-50" : ""
          }`}
          disabled={!isFormatValid() || isLoading}
        >
